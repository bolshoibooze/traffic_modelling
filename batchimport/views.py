...
# Adding check for logged in user to these views, wouldn't want
# just anyone adding whatever they want to the database.
...
def import_execute(request, extra_context={}):
    ...
    **Optional arguments**
    ...
    try:
        ...
        status_dict['error_results_messages'].append('Error opening Excel file: ' + repr(sys.exc_info()[1]))
        return _render_results_response(request, status_dict, extra_context)
    ...
    **Optional arguments**
    ...
    for row in range(process_option_dict['start_row'] - 1, process_option_dict['end_row']):
        ...
        try:
            ...
            status_msg = 'spreadsheet row#' + str(row) + ' ERROR: ' + repr(sys.exc_info()[1])
